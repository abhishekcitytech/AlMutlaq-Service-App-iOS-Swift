...
- (void)layoutSubviews
{
    ...
    _eventIndicator.frame = CGRectMake(self.preferredEventOffset.x,CGRectGetMaxY(_shapeLayer.frame)+eventSize*0.17+self.preferredEventOffset.y,self.titleLabel.frame.size.width,eventSize*0.83);
       ...
       // CGFloat diameter = MIN(MIN(self.fs_width, self.fs_height),FSCalendarMaximumEventDotDiameter);
        for (int i = 0; i < self.eventLayers.count; i++) {
            ...
            if (!eventLayer.hidden) {
                ...
                eventLayer.frame = CGRectMake(-10, self.contentView.frame.size.height-2, 24, 2);
